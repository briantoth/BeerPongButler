...
void imageCallback(const sensor_msgs::ImageConstPtr& original_image)
{
	...
	cvtColor( cv_ptr->image, src_grey, CV_BGR2GRAY );
	...
	for(int i=0; i<cv_ptr->image.rows; i++)
	{
		...
		for(int j=0; j<cv_ptr->image.cols; j++)
		{
			...
			for(int k=0; k<src_grey.channels(); k++)
			{
				//Invert the image by subtracting image data from 255				
				src_grey.data[i*src_grey.rows*4+j*3 + k] = 255-src_grey.data[i*src_grey.rows*4+j*3 + k];
			}
		}
	}
